...
Errors are reported with a concise message and the application remains available for another login attempt. HTTP status codes, network failures, malformed JSON, and JSON responses with an unexpected shape are reported with the response body for diagnosis. Web service output, such as tokens or raw responses, is not printed unless an error has occurred.
...
If login fails, the error is displayed without a stack trace and the user can retry. On a retry, username, password, user SID, and realm are prompted again so the request can be corrected without restarting the application.
